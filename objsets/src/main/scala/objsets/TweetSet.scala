...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    def order(tweets: TweetSet, orderedTweets: Trending): Trending = {
      if (tweets.isEmpty) orderedTweets
      else order(tweets.remove(tweets.findMin), orderedTweets + tweets.findMin)
    }
    order(this, new EmptyTrending)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def addElem(set: TweetSet, acc: TweetSet): TweetSet = {
      if (set.isEmpty) acc
      else addElem(set.tail, acc.incl(set.head))
    }
    addElem(this, that)
  }
